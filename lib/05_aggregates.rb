...
def area_count
  ...
  execute(<<-SQL)
  SELECT COUNT(*) AS countries_over_1mil_pop
  FROM countries
  WHERE area > 1000000
  SQL
end
...
def populous_country_counts
  ...
  execute(<<-SQL)
  SELECT continent, COUNT(name) AS populous_country_counts
  FROM countries
...
def populous_continents
  ...
  execute(<<-SQL)
  SELECT continent AS populous_continents
  FROM countries
  GROUP BY continent
    HAVING SUM(population) > 999999999
  SQL
end
